...
def _local_capture(request: Request, directory: str):
    from urllib.parse import urlsplit
    from src import config as cfg
    from src.pipeline.accepted_capture import capture_images
    # Directory access is local-only and same-origin, unlike image uploads.
    if not request.client or request.client.host not in ("127.0.0.1", "::1", "testclient"):
        raise HTTPException(403, "Local directory input requires a loopback connection")
    origin = request.headers.get("origin")
    if origin and urlsplit(origin).netloc != request.headers.get("host"):
        raise HTTPException(403, "Cross-origin directory input is not allowed")
    try:
        return capture_images(Path(directory), allowed_root=Path(
            os.environ.get("FACE3D_CAPTURE_ROOT", str(cfg.ROOT))))
...
async def runtime_info():
    from src import config as cfg
    return {"pipeline": cfg.RECONSTRUCTION_PIPELINE, "code_root": str(ROOT),
            "calibration": str(cfg.CAMERA_CALIBRATION_PATH)}


@app.post("/api/captures/inspect")
async def inspect_capture(req: CaptureRequest, request: Request):
    from src.pipeline.accepted import preflight
    images = _local_capture(request, req.capture_dir)
    try:
        result = await asyncio.get_running_loop().run_in_executor(None, preflight, images, _manual_intrinsics)
        import base64
        import io
        ...
        for view, path in images.items():
            ...
            result["images"][view]["preview"] = "data:image/jpeg;base64," + base64.b64encode(buffer.getvalue()).decode("ascii")
        return result
    except (OSError, ValueError) as exc:
        raise HTTPException(422, str(exc)) from exc
...
async def create_session(
    ...
    source_images = None
    if capture_dir:
        if any((left_image, front_image, right_image)):
            raise HTTPException(422, "Choose directory input or uploads, not both")
        source_images = _local_capture(request, capture_dir)
        if cfg.RECONSTRUCTION_PIPELINE == "accepted_c481186":
            from src.pipeline.accepted import preflight
            try:
                await asyncio.get_running_loop().run_in_executor(None, preflight, source_images, _manual_intrinsics)
            except (OSError, ValueError) as exc:
                raise HTTPException(422, str(exc)) from exc
    elif not all((left_image, front_image, right_image)):
        raise HTTPException(422, "Three images or one capture directory are required")
    # 创建数据库记录
    sess = SessionModel(
    ...
    for view_name, upload in [
        ...
        if source_images is not None:
            dest = img_dir / f"{view_name}.jpg"
            shutil.copy2(source_images[view_name], dest)
    ...
    session_intrinsics = dict(_manual_intrinsics) if _manual_intrinsics else None
    ...
    async def _run():
        try:
            glb_path = await run_pipeline_async(
                ...
                manual_intrinsics=session_intrinsics,
            )
            async with SessionLocal() as bg_db:
                async with bg_db.begin():
                    ...
                    if row:
                        ...
                        accepted_meta = _session_dir(session_id) / "accepted_c481186" / "run_manifest.json"
                        if accepted_meta.exists():
                            meta_path = accepted_meta
                        if meta_path.exists():
                            with open(meta_path, "r", encoding="utf-8") as f:
                                row.meta = json.load(f)
                        row.finished_at = datetime.utcnow()
            await queue.put({"stage": "done", "pct": 100, "message": "重建完成"})
            ...
            async with SessionLocal() as bg_db:
                async with bg_db.begin():
                    ...
                    if row:
                        ...
                        row.finished_at = datetime.utcnow()
            await queue.put({"stage": "error", "pct": -1, "message": str(exc)})

    async def _launch():
        try:
            await asyncio.gather(_push_progress(session_id, queue), _run())
        finally:
...
async def delete_session(session_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if row.status in ("pending", "running"):
        raise HTTPException(409, "Cannot delete an active reconstruction")
    # 删除文件
