...
def _run_pipeline(
    ...
    if pipeline_name == "accepted_c481186":
        from src.pipeline.accepted import run_accepted_pipeline
        return run_accepted_pipeline(
            session_id=session_id, patient_id=patient_id, image_paths=image_paths,
            session_output_dir=session_output_dir, manual_intrinsics=manual_intrinsics,
            progress=progress,
        )
...
async def run_pipeline_async(
    ...
    def _progress(stage: str, pct: int, message: str):
        if stage == "done":
            return  # API publishes completion only after committing the model path.
