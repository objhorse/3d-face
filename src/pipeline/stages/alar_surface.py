...
def run_multiview_alar_surface_experiment(
    ...
    *,
    ...
    if not result.success:
        (target / "optimization_failure.json").write_text(
            json.dumps({"status": "error", "config": dataclasses.asdict(config),
                        "source_hashes": source_snapshots,
                        "optimization": result.to_report()}, indent=2),
            encoding="utf-8",
        )
