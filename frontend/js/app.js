...
async function init() {
    ...
    document.getElementById('runtime-version').textContent = runtime.pipeline;
    document.getElementById('input-mode').addEventListener('change', updateInputMode);
    document.getElementById('inspect-capture').addEventListener('click', inspectCapture);
    updateInputMode();
...
function handleSessionClick(sess) {
    ...
    hideProgressOverlay();
    document.getElementById('download-model').hidden = true;
    ...
    if (compareMode) {
        ...
        if (sess.has_model) {
            ...
            showRunError(sess.error_msg || '重建失败，请查看运行日志');
        }
    }
}
...
async function loadSingleModel(sessionId) {
    ...
    try {
        ...
        download.href = api.modelUrl(sessionId);
        download.download = `face_${sessionId}.glb`;
...
function watchProgress(sessionId) {
    ...
    const complete = async (session) => {
        if (stopped || !['done', 'error'].includes(session.status)) return;
        stopWatching();
        hideProgressOverlay();
        await refreshSessions();
        if (session.status === 'done') loadSingleModel(sessionId);
        else showRunError(session.error_msg || '请查看运行日志');
    };
    const ws = api.connectProgress(sessionId, (msg) => {
        if (msg.stage === 'ping') return;
        if (msg.stage === 'error') {
            api.getSession(sessionId).then(complete).catch(console.warn);
    ...
    const poll = async () => {
        ...
        polling = true;
        try { await complete(await api.getSession(sessionId)); }
        catch (error) { console.warn(error); }
        finally { polling = false; }
    };
    const timer = setInterval(poll, 3000);
    stopWatching = () => { stopped = true; clearInterval(timer); ws.close(); };
    poll();
}
let stopWatching = null;

function showRunError(message) {
    const panel = document.getElementById('run-error');
...
function updateInputMode() {
    const directory = document.getElementById('input-mode').value === 'directory';
    document.getElementById('directory-input').hidden = !directory;
    document.getElementById('capture-dir').required = directory;
    document.querySelectorAll('.upload-input').forEach(row => {
        row.hidden = directory;
        row.querySelector('input').required = !directory;
    });
    document.getElementById('capture-preview').replaceChildren();
}

async function inspectCapture() {
    const button = document.getElementById('inspect-capture');
    const preview = document.getElementById('capture-preview');
    button.disabled = true;
    preview.textContent = '检查中...';
    try {
        const result = await api.inspectCapture(document.getElementById('capture-dir').value.trim());
        preview.replaceChildren();
        for (const view of ['left', 'front', 'right']) {
            const item = result.images[view];
            const figure = document.createElement('figure');
            const image = document.createElement('img');
            image.src = item.preview;
            image.alt = item.camera;
            const caption = document.createElement('figcaption');
            caption.textContent = `${item.camera}: ${item.size.join(' × ')}`;
...
async function handleUpload(e) {
    ...
    const captureDir = document.getElementById('input-mode').value === 'directory'
        ? form.capture_dir.value.trim() : '';

    if (!patientId || (!captureDir && (!leftFile || !frontFile || !rightFile))) {
    ...
    try {
        const sess = await api.createSession({
            patientId, notes, leftFile, frontFile, rightFile, captureDir,
        });
