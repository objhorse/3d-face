...
export const api = {
    async runtime() {
        const r = await fetch(`${API_BASE}/api/runtime`);
        if (!r.ok) throw new Error(await r.text());
        return r.json();
    },
    async inspectCapture(captureDir) {
        const r = await fetch(`${API_BASE}/api/captures/inspect`, {
    ...
    async createSession({ patientId, notes, leftFile, frontFile, rightFile, captureDir }) {
        const form = new FormData();
        form.append('patient_id', patientId);
        form.append('notes', notes || '');
        if (captureDir) {
            form.append('capture_dir', captureDir);
        } else {
